...
typedef struct {float sdf, emissive, reflectivity;} Result;
...
unsigned char* draw(){
    ...
    for(int y=0; y<H; y++){
        for (int x=0; x<W; x++, p+=3){
            float nx, ny;
            // 这里为何要 这里是将坐标单位化, 圆心是按0.5, 0.5来算的
            p[0] = p[1] = p[2] = (int)fminf(samplePtr((float)x/W, (float)y/H)*255.0f, 255.0f);
    ...
    for(int i=0; i<MAX_STEP && t < MAX_DISTANCE; i++){
        ...
        if(r.sdf < EPSILON){
            ...
            if(depth < MAX_DEPTH && r.reflectivity > 0.0f){
                ...
                sum += r.reflectivity * traceReflect(x + nx * BAIS, y + ny * BAIS, rx, ry, depth + 1);
            }
            return sum;
        }
        t += r.sdf;
